...
This dataset consists of 19 622 observations of 160 variables. Unfortunately, many observations are uncomplete - only `r sum(complete.cases(training))` are actually complete. We also do not need all the variables for the predicitions, so the first step is to clean the dataset.
...
## Model comparison
We built 4 different models, trained on a subset of the training set, and validated on a "validation" subset of the training set.  
